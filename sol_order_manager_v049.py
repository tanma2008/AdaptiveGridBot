...
if FLAG != "1":
    raise RuntimeError(
        "SAFETY STOP: SOL v4.9 requires OKX_FLAG=1 (DEMO)."
    )
...
class OrderManager:
    """OKX spot SOL/USDT adapter for the v4.9 demo loop."""
    ...
    def _instrument_rules(self):
        ...
        # AccountAPI.get_instruments(), matching the existing SOL v4.8 code.
    ...
    def sol_size_from_usdt(self, usdt_size, price):
        ...
        sol_size = self._round_down(usdt_size / price, self.lot_size)
        if sol_size < self.min_size:
            raise ValueError(
                f"Calculated SOL size {sol_size} is below OKX min size {self.min_size}"
            )
        return sol_size
    ...
    def place_limit_order(self, side, price, usdt_size):
        ...
        sol_size = self.sol_size_from_usdt(usdt_size, price)
