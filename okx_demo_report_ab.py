...
def print_comparison(a, b, hours):

    print()
    print("=" * 76)
    print(
        f"             A / B COMPARISON - LAST {hours} HOURS"
    )
    print("=" * 76)
    ...
    rows = [
        ...
        (
            ...
            (
                "INF"
                if a["profit_factor"] is None
                else f"{a['profit_factor']:.4f}"
            ),
            (
                "INF"
                if b["profit_factor"] is None
                else f"{b['profit_factor']:.4f}"
            ),
        ),
        (
            "Avg cycle P/L",
        ...
        (
            "Unmatched BUY BTC",
            f"{a['unmatched_buy']:.8f}",
            f"{b['unmatched_buy']:.8f}",
        ),
    ]
    ...
    diff = b["realized"] - a["realized"]

    print(
        f"P/L difference B-A : "
        f"{diff:+.6f} USDT"
    )
