...
def order_grid_key(order):
    clid = str(order.get("clOrdId", ""))
    match = re.match(r"^BOSS48[A-Z]([BS])(\d{2})", clid)
    if not match:
        return None
...
def order_matches_grid(order, target):
    # Match the logical grid slot, not the old price. Market movement is
    # expected; the existing order must be amended to the new target price.
    key = order_grid_key(order)
    if key is not None:
        return key == grid_key(target)
    return (
...
def amend_order(trade, symbol, order, price, qty=None):
    params = {
        "instId": symbol,
    ...
    if qty is not None:
        params["newSz"] = str(qty)
    return trade._request_with_params("POST", "/api/v5/trade/amend-order", params)
...
def build_plan(bot, market, account, trade, balances, execute=False):
    ...
    for order in ours:
        ...
        if key in by_key:
            ...
            by_key[key] = order
    removed_count = 0
    if execute and duplicates:
        for order in duplicates:
            response = trade._request_with_params("POST", "/api/v5/trade/cancel-order", {"instId": symbol, "ordId": str(order.get("ordId", ""))})
            print(f"  [RECONCILE DUPLICATE] {order.get('clOrdId','')} -> {response}")
            if response.get("code") == "0":
                removed_count += 1
    amended_count = 0
    amend_failed = 0
    for key, order in list(by_key.items()):
        target = target_by_key.get(key)
        if target is None:
            if execute:
                response = trade._request_with_params("POST", "/api/v5/trade/cancel-order", {"instId": symbol, "ordId": str(order.get("ordId", ""))})
                print(f"  [RECONCILE INVALID] {order.get('clOrdId','')} -> {response}")
                if response.get("code") == "0":
                    by_key.pop(key, None)
                    removed_count += 1
            continue
        old_px = D(order.get("px", "0"))
        target_qty = normalize_qty(ORDER_SIZE_USDT, target["price"], info["lot"], info["min_sz"])
        if target_qty is None:
            continue
        new_qty = target_qty if target["side"] == "buy" else D(order.get("sz", "0"))
        if new_qty <= 0:
            new_qty = target_qty
        if execute and (old_px != target["price"] or D(order.get("sz", "0")) != new_qty):
            response = amend_order(trade, symbol, order, target["price"], new_qty)
            print(f"  [RECONCILE AMEND] {order.get('clOrdId','')} {old_px} -> {target['price']} : {response}")
            if response.get("code") == "0":
                order["px"] = str(target["price"])
                order["sz"] = str(new_qty)
                amended_count += 1
            else:
                amend_failed += 1
    if execute:
        post_reconcile_all = get_open_orders(trade, symbol)
        refreshed_ours = bot_open_orders(post_reconcile_all, bot["prefix"])
        if refreshed_ours:
            ours = refreshed_ours
            by_key = {}
        else:
            # Keep the locally reconciled snapshot if OKX temporarily returns an empty list.
            ours = list(by_key.values())
            by_key = {}
        for order in ours:
            key = order_grid_key(order)
        ...
        for order in ours:
            ...
            if key is not None and key not in by_key:
                by_key[key] = order
    exposure = sum((D(o.get("px", "0")) * D(o.get("sz", "0")) for o in by_key.values()), Decimal("0"))
    missing = [g for key, g in target_by_key.items() if key not in by_key]
    ...
    candidates = []
    for entry in valid_buys + valid_sells:
        if len(candidates) >= capacity:
            break
        candidates.append(entry)
    open_buy = sum(1 for o in by_key.values() if o.get("side") == "buy")
    open_sell = sum(1 for o in by_key.values() if o.get("side") == "sell")
    print("-" * 76)
    ...
    print(f"Open bot orders  : {len(by_key)} ({open_buy} BUY / {open_sell} SELL)")
    ...
    print(f"Reconciled       : {removed_count} removed, {amended_count} amended")
    if amend_failed:
    ...
    for g, qty in candidates:
        ...
        print(f"[{mode}] L{g['level']:+d} {g['side'].upper():4} px=${g['price']:,.6f} qty={qty}")
    placed_count = 0
    failed_count = 0
    ours_post = []
    if execute:
        for g, qty in candidates:
            clid = f"BOSS48{bot['id']}{'B' if g['side'] == 'buy' else 'S'}{abs(g['level']):02d}{RUN_ID}"
            params = {"instId": symbol, "tdMode": "cash", "clOrdId": clid, "side": g["side"], "ordType": "limit", "px": str(g["price"]), "sz": str(qty)}
            try:
                ...
                print("     [RECONCILE] Verify the slot on the next cycle before submitting again.")
        ...
        for verify_attempt in range(3):
            try:
                ...
                refreshed_post = bot_open_orders(post_all, bot["prefix"])
                if refreshed_post:
                    ours_post = refreshed_post
                else:
                    # An empty API read immediately after successful amend/place is not
                    # proof that orders disappeared. Preserve the last known good state.
                    ours_post = list(by_key.values())
                post_keys = {order_grid_key(o) for o in ours_post if order_grid_key(o) is not None}
                if set(target_by_key).issubset(post_keys) or verify_attempt == 2:
                    break
                time.sleep(1)
            except Exception as exc:
                post_error = exc
                if verify_attempt < 2:
                    time.sleep(1)
        if post_error and not ours_post:
            print(f"  POST-EXECUTION VERIFY ERROR : {type(post_error).__name__}: {post_error}")
        exposure = sum((D(o.get("px", "0")) * D(o.get("sz", "0")) for o in ours_post), Decimal("0"))
        open_buy = sum(1 for o in ours_post if o.get("side") == "buy")
        open_sell = sum(1 for o in ours_post if o.get("side") == "sell")
        print(f"  POST-EXECUTION   : open={len(ours_post)} ({open_buy} BUY / {open_sell} SELL) | exposure=${exposure:,.2f}")
    final_orders = by_key if not execute else {key: order for order in ours_post if (key := order_grid_key(order)) is not None}
    if execute and len(final_orders) == 0 and (amended_count or placed_count):
        print("  SAFETY VERIFY      : FAIL (successful mutation but no verifiable orders)")
        raise RuntimeError(f"Post-execution verification failed for {symbol}: mutation succeeded but no orders were verifiable")
    final_buy = sum(1 for o in final_orders.values() if o.get("side") == "buy")
    final_sell = sum(1 for o in final_orders.values() if o.get("side") == "sell")
    save_state(bot["state"], {"symbol": symbol, "regime": regime, "grid_percent": str(grid_percent), "anchor": str(live_price), "orders_planned": len(candidates), "open_orders": len(final_orders)})
    return {"id": bot["id"], "symbol": symbol, "orders": len(candidates), "open_orders": len(final_orders), "open_buy": final_buy, "open_sell": final_sell, "regime": regime, "exposure": exposure, "placed": placed_count, "failed": failed_count}
...
def run_cycle(execute=False):
    ...
    print(f"Execution        : {'DEMO EXECUTE' if execute else 'READ ONLY / DRY RUN'}")
    ...
    results = []
    for bot in BOTS:
        results.append(build_plan(bot, market, account, trade, balances, execute=execute))
    ...
    for r in results:
        print(
            f"{r['id']} | {r['symbol']} | {r['regime']:<7} | "
            f"open={r['open_orders']:2d} ({r['open_buy']:2d} BUY/{r['open_sell']:2d} SELL) | "
            f"planned={r['orders']:2d} | exposure=${r['exposure']:,.2f}"
        )
    print("-" * 76)
    total = sum(r["open_orders"] for r in results)
    total_buy = sum(r["open_buy"] for r in results)
    total_sell = sum(r["open_sell"] for r in results)
    print("OPEN ORDERS STATUS")
    for r in results:
        print(f"{r['symbol']} : {r['open_orders']} orders = {r['open_buy']} BUY + {r['open_sell']} SELL")
    print(f"TOTAL    : {total} orders = {total_buy} BUY + {total_sell} SELL")
    return results


def main():
    parser = argparse.ArgumentParser(description="Adaptive Grid v4.8 Boss - BTC/ETH/SOL")
    parser.add_argument("--execute-demo", "--execute", dest="execute_demo", action="store_true", help="submit validated orders to OKX DEMO")
