...
def run_once(): 
    print()
    print("=" * 76)
    print("        ADAPTIVE GRID BOT v4.8 LOOP")
    print("          OKX DEMO CONTINUOUS LOOP")
    print("=" * 76)
...
def loop_main():
    ...
    print("Stop          : Ctrl+C")
    print()
    print("WARNING: Each cycle rebuilds the virtual grid")
    print("around the latest market price.")
    print()
    ...
    try:
        ...
        while True:
            ...
            # Cancel the previous grid before rebuilding it.
            # This prevents duplicate/stale orders when volatility or price
            # moves enough to require a new grid.
            try:
                open_orders = manager.get_open_orders()
                ...
                if open_orders:
                    print()
                    print(
                        f"Existing demo orders : "
                        f"{len(open_orders)}"
                    )
                    print(
                        "Cancelling previous grid..."
                    )

                    cancelled, failed = (
                        cancel_all_demo_orders(
                            manager
                        )
                    )
                ...
                print(
                    f"ORDER CLEANUP ERROR: {exc}"
                )
                time.sleep(interval)
                continue

            # Reuse the tested v4.8 first-run execution.
            try:
                run_once()
        ...
        print()
        print("=" * 76)
        print("        DEMO LOOP STOPPED")
        print("=" * 76)
        print()
        print(
            "Existing OKX Demo orders were NOT "
