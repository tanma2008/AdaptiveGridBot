...
# ============================================================
# ADAPTIVE GRID BOT v4.7 EDGE ANALYSIS
...
def analyze_trade_edge(trades):
    ...
    t["usdt"] = pd.to_numeric(
        t["usdt"],
        errors="coerce",
    ).fillna(0)

    t["execution_price"] = pd.to_numeric(
        t["execution_price"],
        errors="coerce",
    )

    # --------------------------------------------------------
    # Match BUY -> next SELL by grid level.
    # This measures realized grid-cycle economics.
    # --------------------------------------------------------
    ...
    for _, row in t.sort_values(
        ...
        if side == "BUY":
            ...
            # SELL replacement uses the opposite logical level.
            buy_level = -level

            if (
                buy_level in inventory
                and inventory[buy_level]
            ):
    ...
    # --------------------------------------------------------
    # Aggregate by volatility regime
    # --------------------------------------------------------
    ...
    summary_df = pd.DataFrame([{
        ...
        "cycle_pnl": float(total_pnl),
        "cycle_fees": float(total_fees),
        "avg_cycle_pnl": float(
...
def main():
    ...
    print()
    print("=" * 78)
    print("        ADAPTIVE GRID BOT v4.7 EDGE ANALYSIS")
    ...
    result["curve"].to_csv(
        "backtest_v047_equity.csv",
        index=False,
    )

    result["regimes"].to_csv(
        "backtest_v047_regimes.csv",
        index=False,
    )

    result["trades"].to_csv(
        "backtest_v047_trades.csv",
        index=False,
    )

    segments.to_csv(
        "backtest_v047_segments.csv",
        index=False,
    )
    ...
    # Save edge analysis.
    edge["summary"].to_csv(
        "backtest_v047_edge_summary.csv",
        index=False,
    )

    edge["regime"].to_csv(
        "backtest_v047_edge_regime.csv",
        index=False,
    )

    edge["trend"].to_csv(
        "backtest_v047_edge_trend.csv",
        index=False,
    )

    edge["levels"].to_csv(
        "backtest_v047_edge_levels.csv",
        index=False,
    )

    edge["cycles"].to_csv(
        "backtest_v047_grid_cycles.csv",
        index=False,
    )

    print()
    print("Saved:")
    print("  backtest_v047_equity.csv")
    print("  backtest_v047_regimes.csv")
    print("  backtest_v047_trades.csv")
    print("  backtest_v047_segments.csv")
    print("  backtest_v047_edge_summary.csv")
    print("  backtest_v047_edge_regime.csv")
    print("  backtest_v047_edge_trend.csv")
    print("  backtest_v047_edge_levels.csv")
    print("  backtest_v047_grid_cycles.csv")

    print()
    print("=" * 78)
    print("              V4.7 EDGE ANALYSIS COMPLETE")
    print("=" * 78)
    print()
