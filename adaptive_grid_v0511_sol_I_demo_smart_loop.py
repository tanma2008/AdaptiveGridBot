"""
ADAPTIVE GRID BOT I / v5.1.1 - SOL-USDT - OKX DEMO SMART LOOP
...
- I_ENABLE_ORDERS=NO by default => read-only
- Only manages I511 client order IDs
- No automatic cancellation
- Reconciles fills before replacement orders
- Maximum 10 active I orders
...
def stop_handler(signum, frame):
    ...
    running = False
    print()
    print("Stopping I / v5.1.1 safely.")
...
def print_header():
    print("=" * 76)
    print("        ADAPTIVE GRID BOT I / v5.1.1 - SOL-USDT")
...
def main():
    ...
    enabled = os.getenv("I_ENABLE_ORDERS", "NO").upper() == "YES"
    ...
    print(f"SOL avail    : {balance['SOL']:.6f}")
    print(f"All SPOT orders: {len(all_orders)}")
    print(f"I / V5.1.1 orders: {len(bot)}")
    print()

    if not enabled:
        print("READ ONLY CHECK PASSED.")
        print("Set I_ENABLE_ORDERS=YES only when I is approved for Demo orders.")
        return
    ...
    while running:
        try:
            ...
            print(
                ...
                f"SOL={last:,.2f} USDT={balance['USDT']:.2f} "
                f"SOL={balance['SOL']:.5f} I511orders={len(bot)} "
