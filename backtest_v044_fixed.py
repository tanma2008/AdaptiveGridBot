...
class Portfolio:
    ...
    def can_buy(self, mark_price, usdt_size):
        ...
        current_exposure = self.exposure(mark_price)
...
def main():
    ...
    print()
    print("=" * 78)
    print("        ADAPTIVE GRID BOT v4.4 FIXED")
    ...
    result["curve"].to_csv(
        "backtest_v044_equity.csv",
        index=False,
    )

    result["regimes"].to_csv(
        "backtest_v044_regimes.csv",
        index=False,
    )

    result["trades"].to_csv(
        "backtest_v044_trades.csv",
        index=False,
    )
