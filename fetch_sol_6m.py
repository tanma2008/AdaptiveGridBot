...
def fetch(bar, hours, out):
    ...
    while True:
        r = market.get_history_candlesticks(
            instId='SOL-USDT', after=after, bar=bar, limit='300'
        )
        if r.get('code') != '0':
        ...
        rows.extend(data)
        oldest = int(data[-1][0])
        if datetime.fromtimestamp(oldest / 1000, timezone.utc) <= target:
            break
        after = str(oldest)
        if len(data) < 300:
            break
    ...
    for c in ['open','high','low','close','volume','volCcy','volCcyQuote']:
...
fetch('1H', 24 * 190, 'sol_usdt_1h.csv')
fetch('1D', 24 * 190, 'sol_usdt_1d.csv')
